...
class DespachoFormsetTests(TestCase):
    def setUp(self):
        self.material = Materiales.objects.create(nombre="Prod", tipo="Molido")
        self.bodega = Bodegas.objects.create(nombre="Bod")
        self.tercero = Terceros.objects.create(nombre="Cliente", tipo="Cliente")
        self.user = User.objects.create_user(username="tester", password="pass")
        self.lote = Lotes.objects.create(
            numero_lote="LOT-DESP",
            id_material=self.material,
            cantidad_inicial=Decimal("20.00"),
            id_bodega_actual=self.bodega,
        )
        self.despacho = Despacho.objects.create(
            numero_remision="R-DESP",
            tercero=self.tercero,
            direccion_entrega="Dir",
            usuario_creacion=self.user,
        )
        self.detalle = DetalleDespacho.objects.create(
    ...
    def test_formset_valid_without_forms_when_updating(self):
        formset = DetalleDespachoFormSet(
            ...
            instance=self.despacho,
        )
        self.assertTrue(formset.is_valid())

    def test_inventory_movement_on_first_dispatch(self):
        self.client.login(username="tester", password="pass")
        url = reverse('gestion:despacho_update', args=[self.despacho.pk])
        data = {
            'numero_remision': self.despacho.numero_remision,
            'tercero': self.tercero.pk,
            'direccion_entrega': self.despacho.direccion_entrega,
            'estado': 'despachado',
            'observaciones': '',
            'form-TOTAL_FORMS': '1',
            'form-INITIAL_FORMS': '1',
            'form-MIN_NUM_FORMS': '0',
            'form-MAX_NUM_FORMS': '1000',
            'form-0-id': str(self.detalle.pk),
            'form-0-producto': str(self.lote.pk),
            'form-0-cantidad': '5.00',
            'form-0-bodega_origen': str(self.bodega.pk),
        }
        self.client.post(url, data)
        self.lote.refresh_from_db()
        self.assertEqual(self.lote.cantidad_actual, Decimal('15.00'))
        movimientos = MovimientosInventario.objects.filter(id_lote=self.lote, tipo_movimiento='Venta')
        self.assertEqual(movimientos.count(), 1)
        # Ejecutar otra actualización para verificar que no se duplique
        self.client.post(url, data)
        movimientos.refresh_from_db()
        self.assertEqual(movimientos.count(), 1)
